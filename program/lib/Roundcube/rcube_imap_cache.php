...
class rcube_imap_cache
{
     ...
     */
    protected function get_index_row($mailbox)
    {
        // Get index from DB
        $sql_result = $this->db->query(
            "SELECT `data`, `valid`"
     ...
     */
    protected function add_index_row($mailbox, $sort_field,
     ...
     */
    protected function validate($mailbox, $index, &$exists = true)
    {
        $object    = $index['object'];
     ...
     */
    protected function get_index_data($mailbox, $sort_field, $sort_order, $mbox_data = array())
    {
        if (empty($mbox_data)) {
            $mbox_data = $this->imap->folder_data($mailbox);
        }
